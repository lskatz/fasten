...
total_quals=$(echo "$reads_collapsed" | cut -f 3 | tail -n +2 |  awk '{if(NR>1) printf "+"; printf $1}' | bc -l)
...
pe_quals=$(echo "$pe_collapsed" | cut -f 3 | tail -n +2 |  awk '{if(NR>1) printf "+"; printf $1}' | bc -l)
